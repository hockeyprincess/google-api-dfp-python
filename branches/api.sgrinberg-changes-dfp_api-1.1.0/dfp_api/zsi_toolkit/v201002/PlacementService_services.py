...
class PlacementServiceLocator:
    PlacementServiceInterface_address = "https://sandbox.google.com:443/apis/ads/publisher/v201002/PlacementService"
...
class PlacementServiceSoapBindingSOAP:
    ...
    # get: getPlacementsByFilter
    def getPlacementsByFilter(self, request):
        if isinstance(request, getPlacementsByFilterRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getPlacementsByFilterResponse.typecode)
        return response
...
getPlacementsByFilterRequest = ns0.getPlacementsByFilter_Dec().pyclass

getPlacementsByFilterResponse = ns0.getPlacementsByFilterResponse_Dec().pyclass
