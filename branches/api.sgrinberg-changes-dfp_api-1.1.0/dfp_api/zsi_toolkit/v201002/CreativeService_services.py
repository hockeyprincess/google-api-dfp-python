...
class CreativeServiceLocator:
    CreativeServiceInterface_address = "https://sandbox.google.com:443/apis/ads/publisher/v201002/CreativeService"
...
class CreativeServiceSoapBindingSOAP:
    ...
    # get: getCreativesByFilter
    def getCreativesByFilter(self, request):
        if isinstance(request, getCreativesByFilterRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getCreativesByFilterResponse.typecode)
        return response
...
getCreativesByFilterRequest = ns0.getCreativesByFilter_Dec().pyclass

getCreativesByFilterResponse = ns0.getCreativesByFilterResponse_Dec().pyclass
