...
class OrderServiceLocator:
    OrderServiceInterface_address = "https://sandbox.google.com:443/apis/ads/publisher/v201002/OrderService"
...
class OrderServiceSoapBindingSOAP:
    ...
    # get: getOrdersByFilter
    def getOrdersByFilter(self, request):
        if isinstance(request, getOrdersByFilterRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getOrdersByFilterResponse.typecode)
        return response
...
getOrdersByFilterRequest = ns0.getOrdersByFilter_Dec().pyclass

getOrdersByFilterResponse = ns0.getOrdersByFilterResponse_Dec().pyclass
