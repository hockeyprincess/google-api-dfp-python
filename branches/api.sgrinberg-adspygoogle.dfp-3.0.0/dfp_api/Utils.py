...
def GetDataFromCsvFile(file_name):
  """Get data from CSV file, given name of the file from "dfp_api/data".
...
def PurgeLogs(logs_home):
  ...
  This will remove all stored SOAP XML and units data.
  ...
  """
  logs = (os.path.join(logs_home, 'dfp_api_lib.log'),
...
def IsHtml(data):
  ...
  pattern = re.compile('<html>.*?(?:<title>.*?</title>|).*?<body.*?>.*?</body>'
                       '.*?</html>')
