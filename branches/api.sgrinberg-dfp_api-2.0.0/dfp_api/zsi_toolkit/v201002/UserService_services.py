...
class UserServiceLocator:
    UserServiceInterface_address = "https://sandbox.google.com:443/apis/ads/publisher/v201002/UserService"
...
class UserServiceSoapBindingSOAP:
    ...
    def getUsersByFilter(self, request):
        if isinstance(request, getUsersByFilterRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getUsersByFilterResponse.typecode)
        return response
...
getUsersByFilterRequest = ns0.getUsersByFilter_Dec().pyclass

getUsersByFilterResponse = ns0.getUsersByFilterResponse_Dec().pyclass
