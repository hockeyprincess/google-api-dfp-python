...
from dfp_api.Errors import MissingPackageError
from dfp_api.Errors import ValidationError
from dfp_api.zsi_toolkit import MAX_ZSI_VERSION
from dfp_api.zsi_toolkit import MIN_ZSI_VERSION
from dfp_api.zsi_toolkit.HttpConnectionHandler import HttpConnectionHandler
from dfp_api.zsi_toolkit.SigHandler import SigHandler
try:
  import ZSI
...
def SetRequestParams(request, params):
...
def UnpackResponseAsTuple(response):
  ...
  if (isinstance(response, (types.InstanceType, ZSI.TCcompound.ComplexType)) or
    ...
    if (response.__dict__.keys()[0].find('Return') >= 0 or
        ...
        response.__dict__.keys()[0].find('fault') >= 0):
      tpl = ()
...
def PackRequestAsComplexType(request):
  ...
  if isinstance(request, dict):
    ...
    for key in request:
      ...
      data = PackRequestAsComplexType(value)
    ...
    if request:
      ...
      if isinstance(request, dict):
        return PackRequestAsComplexType(request)
      lst = []
      for item in request:
        data = PackRequestAsComplexType(item)
        lst.append(data)
      return lst
