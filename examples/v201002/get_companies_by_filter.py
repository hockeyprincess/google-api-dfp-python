...
"""This code example gets all companies that are advertisers. The filter
...
# Create filter object to only select companies that are advertisers sorted by
# name.
filter = {'text': 'WHERE type = \'ADVERTISER\' ORDER BY name LIMIT 500'}

# Get companies by filter.
companies = company_service.GetCompaniesByFilter(filter)[0]['results']
