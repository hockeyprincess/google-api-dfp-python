...
# Create filter.
filter = 'WHERE lineItemId = \'%s\' AND status = \'ACTIVE\'' % line_item_id

# Get LICAs by filter.
licas = Utils.GetAllEntitiesByFilter(client, 'LineItemCreativeAssociation',
                                     filter)
for lica in licas:
...
result = lica_service.PerformLineItemCreativeAssociationAction(
    {'type': 'DeactivateLineItemCreativeAssociations'}, {'text': filter})[0]
